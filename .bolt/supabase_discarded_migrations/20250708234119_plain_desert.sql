/*
  # Create transactions table for deposits and expenses
  ...
  1. New Tables
    - `transactions`
      ...
      - `payer` (text, for deposits)
      - `payee` (text, for expenses)
    ...
    - `transaction_line_items`
      ...
      - `amount` (numeric)
      - `category_id` (uuid, foreign key to budget_categories)
      - `tax_type` (text, 'none', 'included', 'excluded')
      - `created_at` (timestamp)

  2. Security
    - Enable RLS on both tables
    - Add policies for club admins to manage transactions
    - Add policies for club members to view transactions
*/
