/*
  # Scope archive_member remittance deletion to the club

  1. Problem
    - archive_member deleted ALL of a member's remittance records globally,
      keyed only on member_id. In a multi-club setup this could remove
      remittance history belonging to a different club.

  2. Solution
    - Scope the remittance deletions (and the count) to p_club_id so archiving
      a member in one club never affects another club's records.
*/
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_remittance_count FROM membership_remittances WHERE member_id = p_member_id AND club_id = p_club_id;
  ...
  WHERE membership_remittance_id IN (
    SELECT id FROM membership_remittances WHERE member_id = p_member_id AND club_id = p_club_id
  );
  ...
  DELETE FROM membership_remittances WHERE member_id = p_member_id AND club_id = p_club_id;
