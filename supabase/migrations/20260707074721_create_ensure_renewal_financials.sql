/*
  # ensure_renewal_financials(p_member_id)

  Idempotent helper used both by the renewal "Confirm Payment" flow and by the
  batch backfill. For a financial member it guarantees:

  1. An Association-fee remittance exists covering the member's CURRENT
     membership cycle (renewal_date). If the naturally-computed membership_year
     collides with a previously-PAID remittance from an earlier cycle, the year
     is advanced until a free slot is found (the unique index on
     member_id+club_id+membership_year otherwise silently dropped renewals).
  2. A PAID club-finance income transaction exists for the current cycle.

  Only additive: it never modifies or deletes existing paid records. Safe to run
  repeatedly. Mirrors the primary-payer / association guards used by
  create_remittance_for_membership_v2.
*/
...
BEGIN
  ...
  IF NOT FOUND OR v_member.is_financial IS NOT TRUE THEN
    RETURN jsonb_build_object('skipped', 'not_financial');
  END IF;
