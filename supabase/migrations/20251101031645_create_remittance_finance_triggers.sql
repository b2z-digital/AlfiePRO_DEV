/*
  # Create Finance Transaction Triggers for Remittances

  1. Triggers
    - When club marks remittance as paid to state → Creates club expense + state income
    - When state marks remittance as paid to national → Creates state expense + national income
    - Automatic batch tracking

  2. Transaction Flow
    - Club payment: Creates transactions table entry (existing system)
    - State receipt: Creates association_transactions entry (income)
    - State payment: Creates association_transactions entry (expense for state, income for national)
    
  3. Integration Points
    - Links club transactions to remittances
    - Links association transactions to remittances
    - Complete audit trail from member payment to national receipt
*/

-- Function to create club finance transaction for remittance payment
...
BEGIN
  ...
  IF NEW.club_to_state_status = 'paid' AND (
    ...
    -- Create club expense transaction
    ...
    -- Create state association income transaction
...
-- Function to create state-to-national finance transactions
...
BEGIN
  ...
  IF NEW.state_to_national_status = 'paid' AND (
    ...
    -- Create state expense transaction
    INSERT INTO association_transactions (
      association_id,
      association_type,
      type,
    ...
    -- Create national income transaction
    INSERT INTO association_transactions (
      association_id,
      association_type,
      type,
...
DROP TRIGGER IF EXISTS trigger_club_remittance_payment ON membership_remittances;
DROP TRIGGER IF EXISTS trigger_state_national_remittance_payment ON membership_remittances;

-- Create triggers
CREATE TRIGGER trigger_club_remittance_payment
  AFTER INSERT OR UPDATE OF club_to_state_status ON membership_remittances
  FOR EACH ROW
  EXECUTE FUNCTION create_club_remittance_transaction();

CREATE TRIGGER trigger_state_national_remittance_payment
  AFTER INSERT OR UPDATE OF state_to_national_status ON membership_remittances
  FOR EACH ROW
  EXECUTE FUNCTION create_state_national_remittance_transaction();