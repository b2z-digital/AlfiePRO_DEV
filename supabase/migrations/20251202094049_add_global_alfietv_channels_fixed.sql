/*
  # Add Global AlfieTV Channels Support (Fixed)
  ...
  2. Security
    ...
    - SuperAdmins and State/National Admins can create global channels
    - Only admins can update/delete global channels
*/

-- Add columns for global channel support
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'alfie_tv_channels' AND column_name = 'club_id' AND is_nullable = 'NO') THEN
    ALTER TABLE alfie_tv_channels ALTER COLUMN club_id DROP NOT NULL;
  END IF;
END $$;
...
-- Update insert policy to allow SuperAdmins and State/National Admins to create global channels
...
CREATE POLICY "Admins can insert channels"
  ...
  TO authenticated
  WITH CHECK (
    -- SuperAdmins and Association Admins can create global channels
    (
      is_global = true
      AND (
        EXISTS (
    ...
    (
      (is_global = false OR is_global IS NULL)
...
CREATE POLICY "Admins can update channels"
  ...
  TO authenticated
  USING (
    -- SuperAdmins can update any global channel
    (
      is_global = true
      AND EXISTS (
        SELECT 1 FROM public.profiles
    ...
    OR
    -- State Admins can update global channels
    (
      is_global = true
      AND EXISTS (
        SELECT 1 FROM public.user_state_associations
    ...
    OR
    -- National Admins can update global channels
    (
      is_global = true
      AND EXISTS (
        SELECT 1 FROM public.user_national_associations
...
CREATE POLICY "Admins can delete channels"
  ...
  TO authenticated
  USING (
    -- SuperAdmins can delete any global channel
    (
      is_global = true
      AND EXISTS (
        SELECT 1 FROM public.profiles
    ...
    OR
    -- State Admins can delete global channels
    (
      is_global = true
      AND EXISTS (
        SELECT 1 FROM public.user_state_associations
    ...
    OR
    -- National Admins can delete global channels
    (
      is_global = true
      AND EXISTS (
        SELECT 1 FROM public.user_national_associations
...
DROP POLICY IF EXISTS "Users can view videos from accessible channels" ON alfie_tv_videos;
...
CREATE POLICY "Users can view videos from accessible channels"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.alfie_tv_channels
