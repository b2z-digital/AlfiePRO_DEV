/*
  # Fix simulated event persistence - comprehensive update

  1. Modified Functions
    - `upsert_simulated_race_event(p_data jsonb)` - Updated to handle ALL critical fields
      including scoring_system, observer settings, display settings, roll call, etc.
      Also includes all fields in the ON CONFLICT DO UPDATE clause.

  2. Security
    - Maintains SECURITY DEFINER to bypass RLS SELECT restrictions
    - Still enforces is_simulated = true constraint
    - Verifies caller is authenticated

  3. Important Notes
    - The RPC now accepts and saves: scoring_system, show_flag, show_country,
      show_club_state, show_design, show_category, enable_observers,
      observers_per_heat, enable_roll_call, auto_complete_sail,
      enable_live_tracking, enable_livestream
    - All fields are included in ON CONFLICT DO UPDATE so re-saves persist correctly
    - media and archived fields are also properly handled on update
*/
...
BEGIN
  INSERT INTO public.quick_races (
    ...
    scoring_system, show_flag, show_country, show_club_state, show_design,
