/*
  # Fix Association Transaction Payment Status and Bulk Payments

  1. Issue 1: Payment Status
    - Triggers use 'paid' as payment_status
    - association_transactions table only accepts: 'completed', 'pending', 'failed'
    - Change all triggers to use 'completed' instead of 'paid'

  2. Issue 2: Individual Transactions
    - Currently each remittance creates individual transaction
    - Should create bulk transactions when multiple members paid together
    - Add is_bulk_payment flag and consolidation logic

  3. Security
    - SECURITY DEFINER ensures proper permissions
*/

-- Update club-to-state remittance trigger to use 'completed'
...
BEGIN
  ...
  IF NEW.club_to_state_status = 'paid' AND (
    ...
    -- Create club expense transaction (status: completed for association income)
    ...
    -- Create state association income transaction (use 'completed' status)
...
-- Update state-to-national remittance trigger to use 'completed' and improve bulk handling
...
BEGIN
  ...
  IF NEW.state_to_national_status = 'paid' AND (
    ...
    -- Check if there's an existing batch from today with same payment reference
    -- This allows grouping multiple payments made at the same time
    ...
    IF v_batch_id IS NOT NULL THEN
      ...
      -- Update existing transactions to reflect new totals
...
-- Update existing 'paid' status transactions to 'completed'
UPDATE association_transactions
