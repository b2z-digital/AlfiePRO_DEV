/*
  # Fix Skipper Index Scope Issue
  
  1. Changes
    - Use separate variables for loop counter and matched index
    - Preserve the matched index after loop exits
    - This fixes the null skipper_idx bug
*/
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO trigger_debug_log (event_id, message, details)
    VALUES (NEW.id, 'Searching results', jsonb_build_object('matched_idx', matched_idx, 'max_race', max_race));
    ...
    IF latest_result IS NULL THEN
      INSERT INTO trigger_debug_log (event_id, message, details)
      VALUES (NEW.id, 'No race results', jsonb_build_object('matched_idx', matched_idx));
      CONTINUE;
    END IF;
