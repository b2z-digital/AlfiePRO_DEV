/*
  # Fix Remittance Trigger for Membership Transactions

  The trigger was looking for membership_payments but the app uses membership_transactions.
  This migration creates a new trigger function adapted for membership_transactions schema.
*/

-- Create trigger function for membership_transactions table
...
-- Add trigger to membership_transactions table
DROP TRIGGER IF EXISTS trigger_create_remittance_on_membership_transaction ON public.membership_transactions;
CREATE TRIGGER trigger_create_remittance_on_membership_transaction
  AFTER INSERT OR UPDATE ON public.membership_transactions
  FOR EACH ROW
  WHEN (NEW.payment_status IN ('paid', 'completed'))
  EXECUTE FUNCTION public.create_remittance_from_membership_transaction();
