/*
  # Fix Event Registration Finance Trigger - Payment Status Values

  1. Problem
    - The trigger `create_finance_transaction_from_event_registration` sets status to 'completed' or 'pending'
    - But the transactions table constraint only allows 'paid' or 'awaiting_payment'
    - This causes registration failures when choosing "Pay at Registration"

  2. Solution
    - Update trigger to use correct payment_status values:
      - 'paid' for completed online payments
      - 'awaiting_payment' for pay_at_event registrations

  3. Changes
    - Replace the trigger function with corrected status values
*/

-- Drop and recreate the trigger function with correct payment_status values
...
BEGIN
  ...
  -- FIX: Use 'paid' instead of 'completed' and 'awaiting_payment' instead of 'pending'
