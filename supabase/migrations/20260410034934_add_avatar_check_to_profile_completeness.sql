/*
  # Add avatar check to profile completeness

  1. Changes
    - Updated `check_member_profile_completeness()` to also check if the member
      has an avatar_url set
    - Adds 'avatar' to the missing_fields array when no avatar is present
    - Returns avatar_url in current_data for frontend display
    - Profile is considered incomplete if avatar is missing (in addition to 
      existing phone/emergency/boats checks)

  2. Purpose
    - Encourages members to upload a profile photo during onboarding
    - Provides more complete profile data for the completion banner
