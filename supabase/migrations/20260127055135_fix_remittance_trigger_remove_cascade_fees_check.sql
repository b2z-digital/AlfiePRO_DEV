/*
  # Fix Remittance Trigger - Remove enable_cascade_fees Check
  
  The trigger function was checking for a column (enable_cascade_fees) that doesn't exist
  in the clubs table, causing errors when updating members.
  
  1. Changes
    - Remove the enable_cascade_fees check from create_remittance_for_membership_v2
    - Allow remittances to be created based on association configuration only
*/
...
BEGIN
  ...
  -- Get state association and fees (removed enable_cascade_fees check)
