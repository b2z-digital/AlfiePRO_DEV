/*
  # Fix check_member_linking_status logo column reference

  1. Problem
    - The function references `c.logo_url` but the clubs table column is `c.logo`
    - This causes null club logos to always be returned even when clubs have logos

  2. Fix
    - Change `c.logo_url` to `c.logo` in the clubs join query
*/
