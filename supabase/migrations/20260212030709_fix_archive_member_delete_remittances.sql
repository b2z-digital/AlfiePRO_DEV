/*
  # Fix archive_member to also delete remittance records

  1. Problem
    - Archived/deleted members still appear in Remittances tab
    - membership_remittances rows for the member remain after archiving

  2. Solution
    - Delete pending remittance_reconciliations and remittance_payment_allocations
      linked to the member's remittances
    - Delete membership_remittances for the archived member
    - This runs for all archives (not just full user deletion) since 
      archived members should not have outstanding remittances
*/
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_remittance_count FROM membership_remittances WHERE member_id = p_member_id;
  ...
  WHERE membership_remittance_id IN (
    SELECT id FROM membership_remittances WHERE member_id = p_member_id
  );
  ...
  DELETE FROM membership_remittances WHERE member_id = p_member_id;
  ...
  RETURN jsonb_build_object(
    ...
    'remittances_deleted', v_remittance_count,
