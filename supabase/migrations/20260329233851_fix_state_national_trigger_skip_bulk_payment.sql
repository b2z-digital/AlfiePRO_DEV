/*
  # Fix state-to-national remittance trigger to skip bulk/externally paid records

  1. Problem
    - The `create_state_national_remittance_transaction` trigger fires when
      `state_to_national_status` changes to 'paid', creating finance records
    - When members are marked as "Previously Paid" (paid before the system existed),
      both club-to-state and state-to-national should be marked as paid without
      creating duplicate finance records
    - The club-to-state trigger already checks `bulk_payment = true` to skip,
      but the state-to-national trigger does not

  2. Fix
    - Add `bulk_payment = true` check to the state-to-national trigger so it
      skips creating finance transactions for externally paid members
    - This mirrors the existing behavior in `create_club_remittance_transaction`

  3. Impact
    - "Previously Paid" members will have both remittance legs marked as paid
      without any finance records being created
    - Normal state-to-national payments (bulk_payment = false) are unaffected
*/
