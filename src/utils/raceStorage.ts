      ...
      try {
        ...
        const events: RaceEvent[] = data.map(race => {
          ...
          const raceEvent = {
            ...
            show_country: race.show_country ?? false,
            ...
            showCountry: race.show_country ?? false,
            ...
            proMemberId: race.pro_member_id || undefined,
            proMemberName: race.pro_member_name || undefined
          } as any;
    ...
    if (offlineStorage.getOnlineStatus()) {
      ...
      if ((event as any).is_simulated) {
        const { error } = await supabase.rpc('upsert_simulated_race_event', {
          p_data: {
            ...
            drop_rules: Array.isArray(event.dropRules) ? event.dropRules : (typeof event.dropRules === 'string' ? event.dropRules : [4, 8, 16, 24, 32, 40]),
            ...
            show_country: (event as any).showCountry ?? (event as any).show_country ?? false,
      ...
      const { error } = await supabase
        ...
        .upsert({
          ...
          is_simulated: false,
          pro_member_id: event.proMemberId || null,
          pro_member_name: event.proMemberName || null
        });
      ...
      if (series.rounds && series.rounds.length > 0) {
        ...
        const roundsToUpsert = series.rounds.map((round, index) => {
          const roundData: any = {
            ...
            drop_rules: Array.isArray(round.dropRules) ? round.dropRules : (typeof round.dropRules === 'string' ? round.dropRules : [4, 8, 16, 24, 32, 40]),
...
export const updateEventResults = async (
  ...
  numRaces: number = 12,
  dropRules: number[] | string = [4, 8, 16, 24, 32, 40],
    ...
    try {
      ...
      if (updatedEvent.isSeriesEvent && updatedEvent.seriesId && updatedEvent.roundName) {
        ...
        const roundData: any = {
          ...
          heat_management: heatManagement,
          num_races: numRaces,
          drop_rules: Array.isArray(dropRules) ? dropRules : (typeof dropRules === 'string' ? dropRules : [4, 8, 16, 24, 32, 40]),
