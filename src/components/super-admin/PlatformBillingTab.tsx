...
import { DollarSign, Plus, SquarePen as Edit2, Check, X, Clock, CircleAlert as AlertCircle, CircleCheck as CheckCircle, Receipt, Coins, Calendar, RefreshCw, ChevronDown, ChevronUp, Users, Building2, Globe as Globe2, Target, Mail, Send, Loader as Loader2, MailCheck } from 'lucide-react';
    ...
    try {
      ...
      for (const targetType of ['club', 'state_association', 'national_association']) {
        ...
        for (const entity of allEntities) {
          ...
          const monthlyCharge = parseFloat((memberCount * monthlyRate).toFixed(2));
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                  Projected monthly billing based on active member counts and configured rates
                </p>
              </div>
                  ...
                  <tbody className={`divide-y ${darkMode ? 'divide-slate-700/30' : 'divide-slate-100'}`}>
                    {forecast.filter(f => f.memberCount > 0 || f.monthlyCharge > 0).map(f => (
                      <tr key={`${f.type}-${f.id}`} className={`${darkMode ? 'hover:bg-slate-700/20' : 'hover:bg-slate-50'} transition-colors`}>
                        ...
                        <td className={`p-3 text-right text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                          {formatCurrency(f.annualRate * f.memberCount)}
                        </td>
                      </tr>
                    ))}
                    {forecast.filter(f => f.memberCount > 0).length === 0 && (
                      <tr>
                        <td colSpan={5} className={`p-6 text-center text-sm ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                  ...
                  {forecast.filter(f => f.memberCount > 0).length > 0 && (
                    <tfoot>
                      <tr className={`font-semibold ${darkMode ? 'border-t border-slate-600/50 text-white' : 'border-t-2 border-slate-300 text-slate-900'}`}>
                        ...
                        <td className={`p-3 text-right ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                          {formatCurrency(forecast.reduce((s, f) => s + f.annualRate * f.memberCount, 0))}
                        </td>
                      </tr>
                    </tfoot>
                  )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className={`text-sm font-bold flex-shrink-0 ml-2 ${darkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>
                        {formatCurrency((rate.annual_rate || rate.rate_per_member) / 12)}/mo
                      </p>
                    </div>
                  );
                })}
              ...
              {rateForm.annual_rate > 0 && (
                <div className={`mb-4 p-3 rounded-xl ${darkMode ? 'bg-sky-500/10 border border-sky-500/20' : 'bg-sky-50 border border-sky-200'}`}>
                  <p className={`text-sm ${darkMode ? 'text-sky-300' : 'text-sky-700'}`}>
                    Monthly billing: <span className="font-semibold">{formatCurrency(rateForm.annual_rate / 12)}</span>/member/month
                    ({formatCurrency(rateForm.annual_rate)}/year)
                  </p>
                  <p className={`text-xs mt-1 ${darkMode ? 'text-sky-400/70' : 'text-sky-600'}`}>
                    Existing members are billed at the monthly rate. New members who join mid-cycle are charged the full annual rate to ensure full fee capture.
                  </p>
                </div>
              )}
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <p className={`text-xs ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                        ({formatCurrency(rate.annual_rate || rate.rate_per_member)}/yr per member)
                      </p>
                    </div>
                    <button
                      onClick={() => {
        ...
        <div className="space-y-6">
          <div className={`rounded-2xl border p-6 ${darkMode ? 'bg-slate-800/30 border-slate-700/50' : 'bg-white border-slate-200'}`}>
            ...
            <p className={`text-xs mt-3 ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>
              ...
              Existing members are billed at the monthly rate. New members joining mid-cycle are billed the full annual rate.
            </p>
