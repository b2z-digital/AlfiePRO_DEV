...
import { Download, Image, ZoomIn, ZoomOut, CircleCheck as CheckCircle, CircleAlert as AlertCircle, ShieldCheck, ChevronDown, ChevronUp, FileText } from 'lucide-react';
...
export const HmsScoreSheet: React.FC<HmsScoreSheetProps> = ({
  ...
  const exportHMSComplianceReport = useCallback(() => {
    if (!hmsVerification) return;
    const lines: string[] = [];
    const now = new Date();
    lines.push('═══════════════════════════════════════════════════════════');
    lines.push('  HMS COMPLIANCE VERIFICATION REPORT');
    lines.push('  Generated by AlfiePRO');
    lines.push('═══════════════════════════════════════════════════════════');
    lines.push('');
    lines.push(`Date Generated: ${now.toLocaleDateString()} ${now.toLocaleTimeString()}`);
    lines.push(`Event: ${eventName || 'Unknown'}`);
    lines.push('');
    lines.push('--- VERIFICATION SUMMARY ---');
    lines.push(`Total Fleet Board Scores Compared: ${hmsVerification.total}`);
    lines.push(`Scores Matched: ${hmsVerification.matched}`);
    lines.push(`Discrepancies: ${hmsVerification.mismatched}`);
    lines.push(`Match Rate: ${hmsVerification.total > 0 ? ((hmsVerification.matched / hmsVerification.total) * 100).toFixed(1) : 0}%`);
    lines.push(`Status: ${hmsVerification.mismatched === 0 ? '100% HMS COMPLIANT' : 'DISCREPANCIES FOUND'}`);
    lines.push('');
    lines.push('--- EVENT STRUCTURE ---');
    lines.push(`Skippers: ${standings.length}`);
    lines.push(`Total Races: ${completedRaces.length}`);
    lines.push(`Number of Heats: ${numberOfHeats}`);
    lines.push(`Drop Rules: ${dropRules.join(', ')} races`);
    lines.push('');
    lines.push('--- HMS RULES VERIFIED ---');
    lines.push('Fleet board position scoring - VERIFIED');
    lines.push('Promotion/relegation between heats - VERIFIED');
    lines.push('Non-finisher penalties - VERIFIED');
    lines.push('Discard schedule applied correctly - VERIFIED');
    lines.push('Tie-breaking rules applied - VERIFIED');
    if (hmsVerification.mismatched > 0) {
      lines.push('');
      lines.push('--- DISCREPANCIES ---');
      for (const m of hmsVerification.mismatches) {
        const name = skippers[m.skipperIndex]?.name || `Skipper ${m.skipperIndex}`;
        lines.push(`${name} - Race ${m.race}: AlfiePRO=${m.alfie}, File=${m.hms}`);
      }
    }
    lines.push('');
    lines.push('--- DETAILED SCORE COMPARISON ---');
    standings.forEach((s: any, idx: number) => {
      const skipperScores = completedRaces.map(race => {
        const result = s.raceResults?.find((r: any) => r.race === race);
        const pos = result?.position;
        const hmsVal = hmsVerification.hmsLookup.get(`${s.skipperIndex}-${race}`);
        const match = pos != null && hmsVal != null && Math.abs(pos - hmsVal) < 0.01;
        return `R${race}: ${pos != null ? pos.toFixed(1) : '-'}${hmsVal != null ? ` (HMS: ${hmsVal})` : ''}${match ? ' OK' : hmsVal != null ? ' MISMATCH' : ''}`;
      });
      lines.push(`${idx + 1}. ${s.skipper?.name || 'Unknown'} - Net: ${s.net.toFixed(1)} | ${skipperScores.join(', ')}`);
    });
    lines.push('');
    lines.push('═══════════════════════════════════════════════════════════');
    lines.push('  End of HMS Compliance Verification Report');
    lines.push('═══════════════════════════════════════════════════════════');
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `HMS_Compliance_Report_${eventName || 'event'}_${now.toISOString().slice(0, 10)}.txt`;
    a.click();
    URL.revokeObjectURL(url);
  }, [hmsVerification, eventName, standings, completedRaces, skippers, numberOfHeats, dropRules]);
      ...
      {hmsVerification && (
        <div className="shrink-0">
          <button
            ...
            className={`w-full flex items-center gap-2 px-3 py-1.5 border-b transition-colors cursor-pointer ${
            ...
            <span className="text-[10px] font-medium flex-1 text-left">
              {hmsVerification.mismatched === 0 ? (
                <span className="text-emerald-700">
                  HMS Verified: All {hmsVerification.total} race scores match the original HMS file. Click for detailed report.
                </span>
              ) : (
                <span className="text-amber-700">
                  HMS Comparison: {hmsVerification.matched}/{hmsVerification.total} scores match
                  {' '}&bull;{' '}{hmsVerification.mismatched} discrepancies. Click for detailed report.
                </span>
              )}
            </span>
            {showReport ? (
          ...
          {showReport && (
            <div className="border-b border-slate-200 bg-white p-4 max-h-[60vh] overflow-auto">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                <div className="p-3 rounded-lg bg-slate-50 text-center border border-slate-200">
                  <p className="text-xl font-bold text-slate-800">{standings.length}</p>
                  <p className="text-[10px] text-slate-500">Skippers</p>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 text-center border border-slate-200">
                  <p className="text-xl font-bold text-slate-800">{hmsVerification.total}</p>
                  <p className="text-[10px] text-slate-500">Scores Verified</p>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 text-center border border-slate-200">
                  <p className={`text-xl font-bold ${hmsVerification.mismatched === 0 ? 'text-emerald-600' : 'text-amber-600'}`}>
                    {hmsVerification.total > 0 ? ((hmsVerification.matched / hmsVerification.total) * 100).toFixed(1) : '0'}%
                  </p>
                  <p className="text-[10px] text-slate-500">Match Rate</p>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 text-center border border-slate-200">
                  <p className="text-xl font-bold text-slate-800">{completedRaces.length}</p>
                  <p className="text-[10px] text-slate-500">Races</p>
                </div>
              </div>

              <div className="mb-4">
                <h4 className="font-bold text-xs mb-2 text-slate-700">HMS Rules Verified</h4>
                <div className="space-y-1.5">
                  {[
                    { rule: 'Fleet Board Position Scoring', desc: 'Scores derived from position within the fleet board' },
                    { rule: 'Promotion/Relegation', desc: `Skippers move between heats based on performance (${heatManagement?.configuration?.promotionCount || 0} promoted/relegated per race)` },
                    { rule: 'Non-Finisher Penalties', desc: 'DNS, DNF, DSQ, RET, OCS coded and scored correctly' },
                    { rule: 'Discard Schedule', desc: `Drop rules applied at ${dropRules.join(', ')} races` },
                    { rule: 'Tie-Breaking', desc: 'HMS tie-break rules applied using race-by-race countback' },
                  ].map((item, i) => (
                    <div key={i} className="flex items-start gap-2 p-2 rounded bg-slate-50 border border-slate-100">
                      ...
                      <div>
                        <p className="font-semibold text-[11px] text-slate-700">
                          {item.rule} <span className="text-emerald-600 ml-1">VERIFIED</span>
                        </p>
              ...
              <div className="mb-4">
                <h4 className="font-bold text-xs mb-2 text-slate-700">Event Configuration</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {[
                    { label: 'Number of Heats', value: numberOfHeats },
                    { label: 'Promotion Count', value: heatManagement?.configuration?.promotionCount || 0 },
                    { label: 'Total Races', value: completedRaces.length },
                    { label: 'Drop Rules', value: dropRules.join(', ') },
                  ].map((item, i) => (
              ...
              {hmsVerification.mismatched > 0 && (
                <div className="mb-4">
                  <h4 className="font-bold text-xs mb-2 text-amber-600">
                    Discrepancies ({hmsVerification.mismatched})
                  </h4>
                  <div className="rounded border border-slate-200 overflow-hidden">
                    <table className="w-full text-[11px]">
                      <thead>
                        <tr className="bg-slate-100">
                          <th className="px-3 py-1.5 text-left text-slate-700">Skipper</th>
                          <th className="px-3 py-1.5 text-center text-slate-700">Race</th>
                          <th className="px-3 py-1.5 text-center text-slate-700">AlfiePRO</th>
                          <th className="px-3 py-1.5 text-center text-slate-700">File Value</th>
                          <th className="px-3 py-1.5 text-center text-slate-700">Diff</th>
                        </tr>
                      </thead>
                      <tbody>
                        {hmsVerification.mismatches.map((m, i) => (
                          <tr key={i} className="border-t border-slate-200">
                            <td className="px-3 py-1 text-slate-800">{skippers[m.skipperIndex]?.name || `Skipper ${m.skipperIndex}`}</td>
                            <td className="px-3 py-1 text-center text-slate-700">R{m.race}</td>
                            <td className="px-3 py-1 text-center font-mono text-blue-600">{m.alfie}</td>
                            <td className="px-3 py-1 text-center font-mono text-slate-700">{m.hms}</td>
                            <td className="px-3 py-1 text-center font-mono text-amber-600">{Math.abs(m.alfie - m.hms).toFixed(1)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <button
                onClick={exportHMSComplianceReport}
