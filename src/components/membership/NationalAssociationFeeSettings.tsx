...
interface NationalAssociationFeeSettingsProps {
  darkMode: boolean;
  nationalAssociationId: string;
}

export const NationalAssociationFeeSettings: React.FC<NationalAssociationFeeSettingsProps> = ({
  darkMode,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const structureData = {
        ...
        state_fee_amount: parseFloat(formData.stateFee),
  ...
  const handleEdit = (structure: FeeStructure) => {
    setFormData({
      stateFee: structure.state_fee_amount.toString(),
    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
          State Association Membership Fees
        </h2>
        <p className={`mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          Set the fees that state associations must pay to the national association per member per year
        </p>
      </div>
        ...
        <div className={`p-6 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg border-l-4 border-green-500`}>
          ...
          <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-purple-50'}`}>
            ...
            <p className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
              ${currentStructure.state_fee_amount.toFixed(2)}
            </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className={`block text-sm font-medium mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                State Association Membership Fee ($)
              </label>
              <input
                type="number"
                step="0.01"
                min="0"
                value={formData.stateFee}
                onChange={(e) => setFormData(prev => ({ ...prev, stateFee: e.target.value }))}
                required
              ...
              />
              <p className={`text-xs mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Amount that each state association must pay per member per year
              </p>
            </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <p className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        State Fee: ${structure.state_fee_amount.toFixed(2)}
                      </p>
                      <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'} mt-1`}>
