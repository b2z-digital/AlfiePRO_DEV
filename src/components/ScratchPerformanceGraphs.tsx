...
interface ScratchPerformanceGraphsProps {
  skippers: Skipper[];
  raceResults: any[];
  darkMode: boolean;
}

export const ScratchPerformanceGraphs: React.FC<ScratchPerformanceGraphsProps> = ({
  skippers,
  raceResults,
  darkMode
}) => {
  // Check if Race 1 is complete (all skippers have results)
  const isRace1Complete = skippers.length > 0 && skippers.every((_, index) => 
    raceResults.some(r => r.race === 1 && r.skipperIndex === index)
  );
  ...
  const getNumericPosition = (result: any): number => {
    ...
    // Get number of finishers in this race (excluding letter scores)
  ...
  // Generate unique colors using HSL with fixed saturation and lightness
  ...
  const chartOptions: ChartOptions<'line'> = {
    ...
    elements: {
      ...
      point: { 
        radius: 4, // Slightly smaller points
        hitRadius: 8,
        hoverRadius: 6,
        borderWidth: 1.5 // Thinner point borders
      }
    },
    scales: {
      y: {
  ...
  const getPositionData = () => {
    ...
    const datasets = skippers.map((skipper, skipperIndex) => {
      ...
      return {
        ...
        fill: false,
        borderWidth: 1.5, // Consistent with options
