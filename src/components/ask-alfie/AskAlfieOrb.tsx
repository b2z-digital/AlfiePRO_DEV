...
export const AskAlfieOrb: React.FC<AskAlfieOrbProps> = ({ darkMode }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const pulseAlpha = 0.15 + Math.sin(time * 2) * 0.1;
      ctx.beginPath();
  ...
  return createPortal(
    <>
      <button
        ...
        className={`fixed bottom-6 right-6 z-[9990] w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 hover:scale-110 ${
          isOpen ? 'bg-slate-700 hover:bg-slate-600' : ''
        }`}
        title="Ask Alfie"
        style={!isOpen ? { background: 'transparent', boxShadow: '0 0 20px rgba(56, 189, 248, 0.3)' } : undefined}
      >
        {isOpen ? (
      ...
      {!isOpen && !hasInteracted && (
        <div className="fixed bottom-[5.5rem] right-6 z-[9989] animate-fade-in">
