...
import { X, Trophy, TriangleAlert as AlertTriangle, ShieldCheck, CircleAlert as AlertCircle, ChevronDown, ChevronUp, FileText, Download } from 'lucide-react';
...
const HeatOverallResultsContent: React.FC<HeatOverallResultsModalProps> = ({
  ...
  const exportHMSComplianceReport = () => {
    ...
    a.href = url;
    a.download = `HMS_Compliance_Report_${now.toISOString().slice(0, 10)}.txt`;
    a.click();
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className={`font-bold ${
                    ...
                    {hmsVerification.mismatched === 0
                      ? 'HMS Compliance Verified - 100% Match'
                      : `HMS Verification: ${hmsVerification.mismatched} Discrepancies Found`}
                  </p>
                  <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                    {hmsVerification.matched} of {hmsVerification.total} fleet board scores match imported file values.
                    Click to {showReport ? 'hide' : 'view'} detailed report.
                  </p>
                </div>
              </div>
              {showReport ? (
                <ChevronUp className={darkMode ? 'text-slate-400' : 'text-slate-500'} size={20} />
              ) : (
                <ChevronDown className={darkMode ? 'text-slate-400' : 'text-slate-500'} size={20} />
              )}
            </button>
              ...
              <div className={`mt-4 p-6 rounded-xl border ${
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <div className={`p-4 rounded-lg text-center ${darkMode ? 'bg-slate-800' : 'bg-white'} shadow-sm`}>
                    <p className={`text-2xl font-bold ${darkMode ? 'text-slate-100' : 'text-slate-800'}`}>{skippers.length}</p>
                    <p className={`text-xs ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Skippers</p>
                  </div>
                  <div className={`p-4 rounded-lg text-center ${darkMode ? 'bg-slate-800' : 'bg-white'} shadow-sm`}>
                    <p className={`text-2xl font-bold ${darkMode ? 'text-slate-100' : 'text-slate-800'}`}>{hmsVerification.total}</p>
                    <p className={`text-xs ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Scores Verified</p>
                  </div>
                  <div className={`p-4 rounded-lg text-center ${darkMode ? 'bg-slate-800' : 'bg-white'} shadow-sm`}>
                    <p className={`text-2xl font-bold ${
                      hmsVerification.mismatched === 0 ? 'text-emerald-500' : 'text-amber-500'
                    }`}>
                      {hmsVerification.total > 0 ? ((hmsVerification.matched / hmsVerification.total) * 100).toFixed(1) : '0'}%
                    </p>
                    <p className={`text-xs ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Match Rate</p>
                  </div>
                  <div className={`p-4 rounded-lg text-center ${darkMode ? 'bg-slate-800' : 'bg-white'} shadow-sm`}>
                    <p className={`text-2xl font-bold ${darkMode ? 'text-slate-100' : 'text-slate-800'}`}>{completedRaces.length}</p>
                    <p className={`text-xs ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Races Completed</p>
                  </div>
                </div>

                <div className="mb-6">
                  <h4 className={`font-bold text-sm mb-3 ${darkMode ? 'text-slate-200' : 'text-slate-700'}`}>
                    HMS Rules Verified
                  </h4>
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={i} className={`flex items-start gap-3 p-3 rounded-lg ${
                        ...
                        <div>
                          <p className={`font-semibold text-sm ${darkMode ? 'text-slate-200' : 'text-slate-700'}`}>
                            {item.rule} <span className="text-emerald-500 ml-2">VERIFIED</span>
                          </p>
                ...
                <div className="mb-6">
                  ...
                  <div className={`grid grid-cols-2 md:grid-cols-4 gap-3`}>
                    {[
                      { label: 'Number of Heats', value: heatManagement?.configuration?.numberOfHeats || 2 },
                      ...
                      <div key={i} className={`p-3 rounded-lg ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
                        <p className={`text-xs ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>{item.label}</p>
                        <p className={`font-bold ${darkMode ? 'text-slate-200' : 'text-slate-700'}`}>{item.value}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {hmsVerification.mismatched > 0 && (
                  <div className="mb-6">
                    <h4 className={`font-bold text-sm mb-3 text-amber-500`}>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className={darkMode ? 'bg-slate-800' : 'bg-slate-100'}>
                            <th className={`px-4 py-2 text-left ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>Skipper</th>
                            <th className={`px-4 py-2 text-center ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>Race</th>
                            <th className={`px-4 py-2 text-center ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>AlfiePRO</th>
                            <th className={`px-4 py-2 text-center ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>File Value</th>
                            <th className={`px-4 py-2 text-center ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>Difference</th>
                          </tr>
                        </thead>
                        <tbody>
                          {hmsVerification.mismatches.map((m, i) => (
                            <tr key={i} className={`border-t ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
                              <td className={`px-4 py-2 ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                                {skippers[m.skipperIndex]?.name || `Skipper ${m.skipperIndex}`}
                              </td>
                              <td className={`px-4 py-2 text-center ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>R{m.race}</td>
                              <td className={`px-4 py-2 text-center font-mono ${darkMode ? 'text-blue-400' : 'text-blue-600'}`}>{m.computed}</td>
                              <td className={`px-4 py-2 text-center font-mono ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>{m.imported}</td>
                              <td className={`px-4 py-2 text-center font-mono text-amber-500`}>{Math.abs(m.computed - m.imported).toFixed(1)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
