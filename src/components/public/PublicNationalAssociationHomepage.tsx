...
import { Menu, X, MapPin, ChevronLeft, ChevronRight, Calendar, Clock, Award, Flag } from 'lucide-react';
import { supabase } from '../../utils/supabase';
import { formatDate } from '../../utils/date';

interface NationalAssociation {
  id: string;
  name: string;
  country: string;
...
const DEFAULT_TILES = [
  {
    ...
    description: 'View all state associations in our network',
  ...
  },
  {
    ...
    description: 'View our national championship events',
  ...
  },
  {
    id: 'default-3',
    title: 'Yacht Classes',
    description: 'Explore the yacht classes across the nation',
  ...
  },
  {
    id: 'default-4',
    title: 'News',
    description: 'Stay up to date with national news',
...
export const PublicNationalAssociationHomepage: React.FC = () => {
  const { associationId } = useParams<{ associationId: string }>();
  const [association, setAssociation] = useState<NationalAssociation | null>(null);
  ...
  const loadAssociationData = async () => {
    ...
    try {
      ...
      if (associationData) {
        setAssociation(associationData as NationalAssociation);
      }
  ...
  if (!association) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-white text-xl">National Association not found</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
          ...
          <div className="flex items-center justify-between h-20">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">{association.country}</p>
              </div>
            </div>

            <nav className="hidden md:flex items-center gap-6">
              <a href="#home" className="text-slate-300 hover:text-white transition-colors">Home</a>
              <a href="#states" className="text-slate-300 hover:text-white transition-colors">States</a>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-slate-800 bg-slate-900">
            <nav className="flex flex-col p-4 space-y-2">
              ...
              <a href="#states" className="p-3 text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg transition-colors">States</a>
      ...
      {/* State Associations Section */}
      {states.length > 0 && (
        <section id="states" className="py-16 px-4 sm:px-6 lg:px-8 bg-slate-800/30">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-3xl font-bold mb-8">State Associations</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-6">
              {states.map((state) => (
                ...
                >
                  ...
                  <p className="text-xs text-slate-400">{state.state}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {upcomingEvents.length > 0 && (
        <section id="events" className="py-16 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-3xl font-bold mb-8">Upcoming Championships</h2>
